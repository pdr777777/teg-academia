...
router.post('/iniciar', authMiddleware, async (req, res, next) => {
  try {
    const sessao = await sessaoService.iniciarSessao(req.user.id, req.body.treino_id, 'manual');
...
router.post('/catraca-checkin', async (req, res, next) => {
  try {
    ...
    const sessao = await sessaoService.iniciarSessao(usuarioId, null, 'catraca');
    res.status(201).json(sessao);
  } catch (err) {
    if (err.status) return res.status(err.status).json({ error: err.message });
    next(err);
